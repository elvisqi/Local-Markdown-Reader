...
describe('reloadCurrentDocument', () => {
  it('re-reads the active document and restores the scroll position after rendering', async () => {
    const readMarkdownFile = vi.fn(async () => '# Updated');
    ...
    expect(readMarkdownFile).toHaveBeenCalledWith(expect.objectContaining({ name: 'docs' }), 'README.md');
    expect(renderMarkdown).toHaveBeenCalledWith('# Updated');
